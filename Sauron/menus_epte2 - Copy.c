...
void pantalla_seleccion(){             // Pantallas generales
    ...
    if(bBateriaBaja){
        if(bParpadeoBateria){
            ST7529_printf("     ",69,4,S_,NORMAL); 
        }else{
            sprintf(mystring,"%3u %%",i8Bat);
            ST7529_printf(mystring,69,4,S_,NORMAL);  
        }
    }else{
        sprintf(mystring,"%3u %%",i8Bat);
        ST7529_printf(mystring,69,4,S_,NORMAL);  
    ...
    if(i8Bat>=10){          // Dos o tres digitos
        if(i8Bat>=100){     // Tres digitos
            i8PosVol=POS_VOL_TRES;
        }else{              // Dos digitos
            i8PosVol=POS_VOL_DOS;
        }
    }else{                  // Un dígito
        i8PosVol=POS_VOL_UNO;
    }
    
    if(i16nivel_son==0){
        ST7529_putIcon(ICON_SON_NONE,i8PosVol,0);
    }else if(i16nivel_son<50){
        ST7529_putIcon(ICON_SON_MED,i8PosVol,0);
    }else{
        ST7529_putIcon(ICON_SON_FULL,i8PosVol,0);
    }
...
{        
    if(bRefrescoPantalla){
        if(iMenuAnterior!=0 || bAlMenuPrincipal){
            bAlMenuPrincipal=0;
            ST7529_clear();
            ST7529_printf(sMENU_PRINCIPAL,0,0,L_,NORMAL);
    ...
    */
        ...
        ST7529_printf(sMENU_CONFIG,COLUMNA_1,FILA_INIT,S_,giPos==0);
        ST7529_printf(sMENU_GALV,COLUMNA_1,FILA_1,S_,giPos==1);
        ST7529_printf(sMENU_GALVMICRO,COLUMNA_1,FILA_2,S_,giPos==2);
        ST7529_printf(sMENU_ELECTRO,COLUMNA_1,FILA_3,S_,giPos==3);
        ST7529_printf(sMENU_SMP,COLUMNA_1,FILA_4,S_,giPos==4);
        ST7529_printf(sMENU_TEST,COLUMNA_1,FILA_5,S_,giPos==5);
    
        iMenuAnterior=iMenuActual;
    }
    
    bRefrescoPantalla=0;return 0;
}

int8 menu_10()   // ConfiuraciÃ³n dispositivo 
{

   if(iMenuAnterior!=10)
   {
      ST7529_clear();
      ST7529_printf(sCONFIG,0,0,L_,NORMAL);
   } 
    if(bRefrescoPantalla){
       pantalla_seleccion();
       ...
       if(giPos>=5)
       {
        ...
        ST7529_printf(sCONFIG_ILUM,6,FILA_INIT,S_,giPos==0);
        ST7529_printf(sCONFIG_CONT,6,FILA_1,S_,giPos==1); 
        ST7529_printf(sCONFIG_PITO,6,FILA_2,S_,giPos==2);
        ST7529_printf(sCONFIG_IDIO,6,FILA_3,S_,giPos==3); 
        ST7529_printf(sCONFIG_VERS,6,FILA_4,S_,giPos==4);
...
int8 menu_20()   // Galvanica + microcorrientes
{
    if(bRefrescoPantalla){
       ...
       ST7529_printf(sGALV,5+ (COLUMNA_1+COLUMNA_2)/2,FILA_INIT-SALTO_FILA,S_,0);
       
       sprintf(mystring,"C=%6lu[C",(int32)i32CGalv+i16CRampaGalv);
       ST7529_printfg(mystring,COLUMNA_1,FILA_INIT,M_,ai8EscalaGrises[1],0);
       
       sprintf(mystring,"I=%5lu[A",i16Igalv*50);
       ...
       if(i16Tgalv==0){
        ...
        sprintf(mystring,"Ta=%2u:%02u s",i8Tmin,i8Tseg);
       }
       ST7529_printf(mystring,COLUMNA_1,FILA_1,M_,giPos==2);
       
       sprintf(mystring,"Tr=%2u s",i8Trampagalv);
       ST7529_printf(mystring,COLUMNA_2,FILA_1,M_,giPos==3);
       
       if(bSeleccionGalvanica){
        
           ST7529_printf(sMICRO_CORRIENTES,(COLUMNA_1+COLUMNA_2)/2,FILA_2,S_,0);
           
           //sprintf(mystring,"C=%05luuC",i16Cmicro);
           //ST7529_printf(mystring,10,72,M_,0);
        
           sprintf(mystring,"I=%4lu[A",i16Imicro*50);
           ...
           ST7529_printf(i8RepVal1,COLUMNA_1,FILA_4,M_,giPos==6);
          
           sprintf(mystring,"PW=%4lu [s",i16PWmicro);
           ST7529_printf(mystring,COLUMNA_2,FILA_4,M_,giPos==7);
        
           sprintf(mystring,"F=%4luHz",i16frecmicro);
           ST7529_printf(mystring,COLUMNA_1,FILA_5,M_,giPos==8);   
       
       }
     
       ST7529_printf(sCONFIG_TRAT,COLUMNA_1,FILA_RELISTO,M_,giPos==9);
       iMenuAnterior=iMenuActual;
    }
    bRefrescoPantalla=0;return 0;
}
...
int8 menu_40(){
    ...
    switch(i8ConfigCanal){
        ...
        case CHC_C:
            ...
            break;
        case POINTER_C:
            i8RepVal1=sCANAL_POINTER;             // Muestra el texto del canal C
            break;
    }
    ST7529_printf(i8RepVal1,23,18,M_,(giPos==0));                // Muestra por pantalla los títulos

    Calc_representacion_t(i16TacElec[i8ConfigCanal],i16TdElec[i8ConfigCanal]);
    comprobacionLimitesFrecuencia();
    
    // Texto menus
    if(POINTER_C != i8ConfigCanal){         // Si no estamos en el menu de configuración del pointer
        // Saltos menú
        if(!bPolaridadElec[i8ConfigCanal]){                  // Si la señal es monopolar
            if(!bascendente){                               // Si estamos descendiendo en el menú
                if(giPos==11){                          // Si estamos en la configuración del PW-
                ...
                if(giPos==11){                          // Si estamos en el menú de configuracion del PW-
                    giPos=10;                            // Salto a PW+
                }
                if(giPos==9){
                    giPos=8;
        ...
        switch(i8SimetriaCanales){
            case 0:
                i8RepVal1=sTextoSimetriaCanalesNO;             
                break;
            case 1:
                i8RepVal1=sTextoSimetriaCanalesAB;            
                break;
            case 2:
                i8RepVal1=sTextoSimetriaCanalesABC;             
                break;
        }
        ST7529_printf(i8RepVal1,COLUMNA_1,FILA_INIT,M_,(giPos==1));   
        ...
        ST7529_printf(mystring,COLUMNA_1,FILA_1,M_,(giPos==2));             
        sprintf(mystring,"Tr=%2u s   ",i8TrElec[i8ConfigCanal]);              // Tiempo Rampa
        ST7529_printf(mystring,COLUMNA_1,FILA_2,M_,(giPos==4));
        
        sprintf(mystring,"Td=%2u:%02u s",i8T1min,i8T1seg);                // Tiempo Descanso/Reposo
        ST7529_printf(mystring,COLUMNA_2,FILA_1,M_,(giPos==3));
        ...
        ST7529_printfg(i8RepVal1,COLUMNA_1,FILA_4,M_,ai8EscalaGrises[bCondicion],giPos==8);          // Muestra por pantalla la polaridad

        if(bSimetriaElec[i8ConfigCanal]){                                       // Si la señal es simétrica
            i8RepVal1=sSIMETRICA;                                       // Indico simetría 
            //bPolaridadElec[i8ConfigCanal]=1;                                    // Pongo señal bipolar (Es preciso?)
        ...
        ST7529_printfg(i8RepVal1,COLUMNA_2,FILA_4,M_,ai8EscalaGrises[bCondicion],giPos==9);         // Muestra por pantalla la simetría 
        ...
        sprintf(mystring,"PW-=%4lu[s",i16PW_minusElec[i8ConfigCanal]);     // PW- coge el valor del PW+                                                                                           // Si debe estar habilitado
    ...
    ST7529_printf(sCONFIG_TRAT,COLUMNA_1,FILA_RELISTO,M_,giPos==12);        // Activo botón relisto
...
if(bRefrescoPantalla){
    ...
    sprintf(mystring,"Td=%5lu uS",i32TDesfaseSMP);          // Tiempo Desfase pulsos
    ...
    ST7529_printf(sCONFIG_TRAT,COLUMNA_1,FILA_RELISTO,M_,giPos==4);         // Activo botón relisto
...
int8 menu_14(){
    ...
    if(bRefrescoPantalla){
        if (iMenuAnterior!=12){          // Si no viene de ninguno de los menús indicados
            ST7529_clear();                                     // Limpio la pantalla   
            ST7529_printf(sCONFIG_CONT,0,0,L_,NORMAL);        // Muestro texto 
...
int8 menu_15(){
    if(bRefrescoPantalla){
        ...
        switch(giPos){
            case SPANISH:
                sprintf(mystring,"Español");                   // Muestro valor por pantalla
        ...
        ST7529_printf(mystring,20,50,L_,0);
...
if(bRefrescoPantalla){
   ...
   sprintf(mystring,"%6LU uC",(int32)giPos*100);
...
if(bRefrescoPantalla){
   ...
   sprintf(mystring,"%4lu us",giPos*25);
   ...
   switch (iMenuActual)
   {
      ...
      case 51:
      {
         ...
         /*maxBW=Calc_BWmax(i16frecElec[i8ConfigCanal],bpolaridadElec[i8ConfigCanal]);
         if(bPolaridadElec[i8ConfigCanal]){
            if((i16PW_plusElec[i8ConfigCanal]+i16PW_minusElec[i8ConfigCanal])>redondeo25((0.9*1000000/i16FrecElec[i8ConfigCanal]))){
                maxBW=(i16PW_plusElec[i8ConfigCanal]-25)/25;
                if(i16PW_plusElec[i8ConfigCanal]>=((25*maxBW)-i16PW_minusElec[i8ConfigCanal])){
                    i16PW_plusElec[i8ConfigCanal]=((25*maxBW)-i16PW_minusElec[i8ConfigCanal]);
                    maxBW=i16PW_plusElec[i8ConfigCanal]/25;
                }
            }
         }*/
      ...
      case 52:
      {
         ...
         if(bPolaridadElec[i8ConfigCanal]){
            if((i16PW_plusElec[i8ConfigCanal]+i16PW_minusElec[i8ConfigCanal])>redondeo25((0.9*1000000/i16FrecElec[i8ConfigCanal]))){
                maxBW=(i16PW_minusElec[i8ConfigCanal]-25)/25;
                if(i16PW_minusElec[i8ConfigCanal]>=((25*maxBW)-i16PW_plusElec[i8ConfigCanal])){
                    i16PW_minusElec[i8ConfigCanal]=((25*maxBW)-i16PW_plusElec[i8ConfigCanal]);
                    maxBW=i16PW_minusElec[i8ConfigCanal]/25;
                }
            }
         }*/
         if(i8SimetriaCanales!=0){
...
// Simetria de la señal (Pointer + Elecroestimulacion)
...
// Selección canal electroestimulacion
int8 menu_41(){
if(bRefrescoPantalla){
    ...
    if(bRefrescoPantalla){
        ...
        switch (iMenuActual){
            ...
            case 82:        // Desfase SMP
                ...
                sprintf(mystring1,"Td=%6luus",i32TDesfaseSMP);
...
int8 menu_61(){
if(bRefrescoPantalla){
    if (iMenuAnterior!=61){                         // Si es la primera vez que entramos en este menú
        ST7529_clear();                             // Limpio la pantalla
        ST7529_printf(sTEST_LEDS,0,0,L_,NORMAL);     // Imprimo por pantalla
        giPos=0;                                    // Inicializo giPos
    } 

    write_PCA955X(0x00,0x00,0x00,PCA9555);          // Envia el dato completo al expansor (apagar leds sin molestar a los relés)
    pantalla_seleccion();                           // Muestra los iconos de selección           
    ...
    iMenuAnterior=iMenuActual;                      // Asigno menú actual a menú anterior
}
        bRefrescoPantalla=0;return 0;
}
...
/*
* Test BQ34Z1000G1
* Comprueba el tipo de dispositivo y la tensión medida
*/
int8 menu_71()                       
    {
    if(bRefrescoPantalla){
    ...
    ST7529_printf(i8RepVal1,10,15,S_,0);

    sprintf(mystring,"Voltage: %4Ld mV",getVoltage());                        // Obtención de la tensión de la batería 
    ST7529_printf(mystring,10,25,S_,0);
    sprintf(mystring,"Current: %4Ld mA",getCurrent());                        // Obtennción de la corriente que pasa por el shunt
    ST7529_printf(mystring,10,35,S_,0);
    sprintf(mystring,"Capacity: %Ld/%Ld mAh",getRemaining(),getCapacity());    // Obtención de la capacidad restante / total de la bateria
    ST7529_printf(mystring,10,45,S_,0);
    sprintf(mystring,"Percentage: %u %%",nivelBateria());                      // Porcentaje del nivel de batería restante
    ST7529_printf(mystring,10,55,S_,0);
    
    if(estadoCarga()){                                                         // indica si el equipo está conectado a la red eléctrica
    ...
    sprintf(mystring,"Temperature: %3.1w C",getTemp());                        // Obtención de la temperatura del sensor de temperatura interno (o externo?)
    ST7529_printf(mystring,10,75,S_,0);
    sprintf(mystring,"Flags:   0x%LX",getFlags());                             // Obtención de los flags de control
    ST7529_printf(mystring,10,85,S_,0);
    sprintf(mystring,"FlagsB:  0x%LX",getFlagsB());                            // Obtención de los flags de control
    ST7529_printf(mystring,10,95,S_,0);
    sprintf(mystring,"Cycle Count:  %Lu",getCycleCount());                     // Número de ciclos de craga de la batería
...
{ 
if(bRefrescoPantalla){
    ST7529_clear();                                 // Se ha eliminado la condición IF porque quiero que siempre limipe la pantalla
    ...
    delay_ms(3000);                                     // Realiza lla comunicación cada 3 segundos*/
}
        bRefrescoPantalla=0;return 0;
} 
...
int8 menu_120(){
    ...
    if(iMenuActual!=123){        // Si no hay error de electrodo
        ...
        /*
        // Si el menú anterior es la pantalla de tratamiento y el tratamiento de galvanica no está activado 
        if(!bPausaTratamiento && iMenuAnterior==120 && !bTratamientoGalvanicaActivado){
            // Asigno el valor de corriente galvánica a giPos
            //giPos=i16Igalv;
        }*/
...
int8 menu_140(){
    ...
    if(iMenuActual!=143){               // Si no hay error de electrodo
        ...
        if(iMenuAnterior!=140 && iMenuAnterior!=141 && iMenuAnterior!=142){                // Si es la primera vez que entra al menu de terapia
            ...
            pantalla_seleccion();                                                           // Añado botones control
       ...
       if(i8ConfigCanal!=POINTER_C){
        ...
        Calc_representacion_t(i16TimeRaw[i8ConfigCanal],i16TimeRepRaw[i8ConfigCanal]);                           // Cálcula de nuevo la representación del tiempo
        i8Time_min=i8Tmin;                                                         // Recarga de nuevo las variables para usarlas de nuevo en el caso de error de electrodo
        i8Time_seg=i8Tseg;            
        i8Time1_min=i8T1min;
        i8Time1_seg=i8T1seg;
...
if(bRefrescoPantalla){
    ...
    if(iMenuActual!=183){ // Si no hay erro de electrodo
        ...
        if((iMenuAnterior==80 && iMenuActual==180)){        // Envio el paquete de datos si es la primera vez que entra al menú
            ...
            structTratamientoMultipulsosActivo.si16ISMP=i16ISMP;              // Configuración corriente en el sistema multipulsos (activo)
            structTratamientoMultipulsosActivo.si8PulsosSMP=i8PulsosSMP;      // Configuraciión pulsos en el sistema multipulsos (activo)
...
// Menú indicación de autoapagado
int8 menu_200()   
{
...
int8 menu_204()
{
   ...
   // Datos de test - Estos datos se sustituirán por un nivel de porcetaje o un dibujo de una bateria en carga
...
// Menú de error de comunicaciones
int8 menu_205(){
